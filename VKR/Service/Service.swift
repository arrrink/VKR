...
struct DriverService {
    ...
    func observeTrips(completion: @escaping(Trip) -> Void) {
        REF_TRIPS.observe(.childAdded) { (snapshot) in
            ...
            let trip = Trip(clientUid: uid, dictionary: dictionary)
            completion(trip)
        }
    }
    ...
    func observeTripCancelled(trip: Trip, completion: @escaping() -> Void) {
        REF_TRIPS.child(trip.clientUid).observeSingleEvent(of: .childRemoved) { _ in
            completion()
        }
    }
    ...
    func acceptTrip(trip: Trip, completion: @escaping(Error?, DatabaseReference) -> Void) {
        ...
        REF_TRIPS.child(trip.clientUid).updateChildValues(values, withCompletionBlock: completion)
    }
    
    func updateTripState(trip: Trip, state: TripState,
                         completion: @escaping(Error?, DatabaseReference) -> Void) {
        REF_TRIPS.child(trip.clientUid).child("state").setValue(state.rawValue, withCompletionBlock: completion)
        
        if state == .completed {
            REF_TRIPS.child(trip.clientUid).removeAllObservers()
        }
    }
...
struct ClientService {
    ...
    func fetchDrivers(location: CLLocation, completion: @escaping(User) -> Void) {
        ...
        REF_DRIVER_LOCATIONS.observe(.value) { (snapshot) in
            geofire.query(at: location, withRadius: 70).observe(.keyEntered, with: { (uid, location) in
