...
enum LocationType: Int, CaseIterable, CustomStringConvertible {
    ...
    var description: String {
        switch self {
        case .home: return "Работа"
        case .work: return "Склад"
        }
    }
    
    var subtitle: String {
        switch self {
        case .home: return "Добавить Работу"
        case .work: return "Добавить Склад"
        }
    }
}
...
class SettingsController: UITableViewController {
    ...
    func configureNavigationBar() {
        ...
        navigationItem.leftBarButtonItem = UIBarButtonItem(image: #imageLiteral(resourceName: "menu").withRenderingMode(.alwaysOriginal), style: .plain, target: self, action: #selector(handleDismissal))
    }
}
...
extension SettingsController: AddLocationControllerDelegate {
    func updateLocation(locationString: String, type: LocationType) {
        ClientService.shared.saveLocation(locationString: locationString, type: type) { (err, ref) in
            self.dismiss(animated: true, completion: nil)
